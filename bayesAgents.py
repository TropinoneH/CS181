...
from layout import PROB_BOTH_BOTTOM, PROB_BOTH_TOP, PROB_FOOD_RED, PROB_GHOST_RED, PROB_ONLY_LEFT_BOTTOM, PROB_ONLY_LEFT_TOP
...
def fillHouseCPT(bayesNet: BayesNet, gameState: GameState):
    ...
    for assignment in foodHouseFactor.getAllPossibleAssignmentDicts():
        ...
        top = assignment[Y_POS_VAR] == BOTH_TOP_VAL or (left and assignment[Y_POS_VAR] == LEFT_TOP_VAL)

        if (
            top
            and left
            and assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL
            or top
            and not left
            and assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL
            or not top
            and left
            and assignment[FOOD_HOUSE_VAR] == BOTTOM_LEFT_VAL
            or not top
            and not left
            and assignment[FOOD_HOUSE_VAR] == BOTTOM_RIGHT_VAL
        ):
            prob = 1
        else:
            prob = 0
    ...
    for assignment in ghostHouseFactor.getAllPossibleAssignmentDicts():
        ...
        if (
            top
            and left
            and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL
            or top
            and not left
            and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL
            or not top
            and left
            and assignment[GHOST_HOUSE_VAR] == BOTTOM_LEFT_VAL
            or not top
            and not left
            and assignment[GHOST_HOUSE_VAR] == BOTTOM_RIGHT_VAL
        ):
            prob = 1
        else:
            prob = 0
...
def fillObsCPT(bayesNet: BayesNet, gameState: GameState):
    ...
    coordToString = {bottomLeftPos: BOTTOM_LEFT_VAL, topLeftPos: TOP_LEFT_VAL, bottomRightPos: BOTTOM_RIGHT_VAL, topRightPos: TOP_RIGHT_VAL}
        ...
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in assignments:
                ...
                if houseVal != ghostHouseVal and houseVal != foodHouseVal:
                    newObsFactor.setProbability({obsVar: RED_OBS_VAL, GHOST_HOUSE_VAR: ghostHouseVal, FOOD_HOUSE_VAR: foodHouseVal}, 0)
                    newObsFactor.setProbability({obsVar: BLUE_OBS_VAL, GHOST_HOUSE_VAR: ghostHouseVal, FOOD_HOUSE_VAR: foodHouseVal}, 0)
                    newObsFactor.setProbability({obsVar: NO_OBS_VAL, GHOST_HOUSE_VAR: ghostHouseVal, FOOD_HOUSE_VAR: foodHouseVal}, 1)
                else:
                    if houseVal == ghostHouseVal and houseVal == foodHouseVal:
                    ...
                    newObsFactor.setProbability({obsVar: RED_OBS_VAL, GHOST_HOUSE_VAR: ghostHouseVal, FOOD_HOUSE_VAR: foodHouseVal}, prob_red)
                    newObsFactor.setProbability({obsVar: BLUE_OBS_VAL, GHOST_HOUSE_VAR: ghostHouseVal, FOOD_HOUSE_VAR: foodHouseVal}, prob_blue)
                    newObsFactor.setProbability({obsVar: NO_OBS_VAL, GHOST_HOUSE_VAR: ghostHouseVal, FOOD_HOUSE_VAR: foodHouseVal}, 0)
...
def getMostLikelyFoodHousePosition(evidence: Dict, bayesNet: BayesNet, eliminationOrder: List[str]):
    ...
    prob_dict = [(factor.getProbability({FOOD_HOUSE_VAR: val} | evidence), {FOOD_HOUSE_VAR: val}) for val in HOUSE_VALS]
    return max(prob_dict, key=lambda x: x[0])[1]
    "*** END YOUR CODE HERE ***"
...
class BayesAgent(game.Agent):
    ...
    def getEvidence(self, gameState):
        ...
        for ePos, eColor in gameState.getEvidence().items():
            ...
            obsVal = {"B": BLUE_OBS_VAL, "R": RED_OBS_VAL, " ": NO_OBS_VAL}[eColor]
    ...
    def goToBest(self, gameState):
        ...
        bestFoodAssignment = getMostLikelyFoodHousePosition(evidence, self.bayesNet, eliminationOrder)

        tx, ty = dict(zip([BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL], gameState.getPossibleHouses()))[bestFoodAssignment[FOOD_HOUSE_VAR]]
        bestAction = None
        bestDist = float("inf")
...
class VPIAgent(BayesAgent):
    ...
    def __init__(self):
        ...
        self.exploreActionsRemaining = list(reversed([NORTH, NORTH, NORTH, NORTH, EAST, EAST, EAST, EAST, SOUTH, SOUTH, SOUTH, SOUTH, WEST, WEST, WEST, WEST]))
    ...
    def getExplorationProbsAndOutcomes(self, evidence):
        ...
        for foodHouseVal, ghostHouseVal in [(TOP_LEFT_VAL, TOP_RIGHT_VAL), (TOP_RIGHT_VAL, TOP_LEFT_VAL)]:

            condEvidence = dict(evidence)
            condEvidence.update({FOOD_HOUSE_VAR: foodHouseVal, GHOST_HOUSE_VAR: ghostHouseVal})
            assignmentProb = houseMarginals.getProbability(condEvidence)

            oneObsMarginal = inference.inferenceByVariableElimination(self.bayesNet, [firstUnk], condEvidence, restUnk + [X_POS_VAR, Y_POS_VAR])
            ...
            for nRed in range(8):
                outcomeProb = combinations(7, nRed) * redProb**nRed * (1 - redProb) ** (7 - nRed)
    ...
    def getAction(self, gameState):
        ...
        if self.behavior == None:
            ...
            enterLeftValue, enterRightValue = self.computeEnterValues(evidence, enterEliminationOrder)
            exploreValue = self.computeExploreValue(evidence, exploreEliminationOrder)
    ...
    def enterAction(self, gameState, left=True):
        ...
        best = min(zip(actions, neighborStates), key=lambda x: self.distances[x[1], dest])
        return best[0]
    ...
    def exploreAction(self, gameState):
        ...
        evidence = self.getEvidence(gameState)
        enterLeftValue, enterRightValue = self.computeEnterValues(evidence, [X_POS_VAR, Y_POS_VAR])
