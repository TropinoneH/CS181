...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin([~literals[i] | ~literals[j] for i, j in itertools.combinations(range(len(literals)), 2)])
    "*** END YOUR CODE HERE ***"
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    end = PropSymbolExpr(pacman_str, x1, y1, time=1)
    return findModel(conjoin(conjoin(KB), end)), findModel(conjoin(conjoin(KB), ~end))
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        print(f"-------{t:02d}--------")
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords]))
        model = findModel(conjoin(conjoin(KB), conjoin([~PropSymbolExpr(food_str, x, y, time=t) for x, y in food])))
        if model:
            return extractActionSequence(model, actions)
        KB.append(exactlyOne([PropSymbolExpr(action, time=t) for action in actions]))
        KB.extend([pacmanSuccessorAxiomSingle(x, y, t+1, walls) for x, y in non_wall_coords])
        KB.extend([PropSymbolExpr(food_str, x, y, time=t+1) % conjoin(PropSymbolExpr(food_str, x, y, time=t), ~PropSymbolExpr(pacman_str, x, y, time=t)) for x, y in food])
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            cur_state = PropSymbolExpr(pacman_str, x, y, time=t)
            if findModel(conjoin(conjoin(KB), cur_state)) is not False:
                possible_locations.append((x, y))
            elif entails(conjoin(KB), cur_state):
                KB.append(cur_state)
            elif entails(conjoin(KB), ~cur_state):
                KB.append(~cur_state)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))
        for x, y in non_outer_wall_coords:
            now = PropSymbolExpr(wall_str, x, y)
            if entails(conjoin(KB), now):
                KB.append(now)
                if known_map[x][y] == -1:
                    known_map[x][y] = 1
            elif entails(conjoin(KB), ~now):
                KB.append(~now)
                if known_map[x][y] == -1:
                    known_map[x][y] = 0
        agent.moveToNextState(agent.actions[t])
...
def slam(problem, agent) -> Generator:
    ...
    #                 if known_map[x][y] == -1:
    #                     known_map[x][y] = 0
    #         now = PropSymbolExpr(pacman_str, x, y, time=t)
    #         if findModel(conjoin(conjoin(KB), now)) is not False:
    #             possible_locations.append((x, y))
    #         if entails(conjoin(KB), now):
    #             KB.append(now)
    #         else:
