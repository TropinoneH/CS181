...
class InferenceModule:
    ...
    def getPositionDistributionHelper(self, gameState, pos, index, agent):
        ...
        if pacmanPosition == ghostPosition:  # The ghost has been caught!
            dist[jail] = 1.0
            return dist
        pacmanSuccessorStates = game.Actions.getLegalNeighbors(pacmanPosition, gameState.getWalls())  # Positions Pacman can move to
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for pos in self.legalPositions:
            self.particles.extend([pos] * (self.numParticles // len(self.legalPositions)))
    ...
    def observeUpdate(self, observation: Union[int, float, None], gameState: busters.GameState):
        ...
        jail_pos = self.getJailPosition()
        weights = [self.getObservationProb(observation, pos, particle, jail_pos) for particle in self.particles]
        if sum(weights) == 0:
            self.initializeUniformly(gameState)
        else:
            belief = DiscreteDistribution()
            ...
            for i, particle in enumerate(self.particles):
                if particle not in belief.keys():
                    belief[particle] = weights[i]
                else:
                    belief[particle] += weights[i]
            belief.normalize()
            # Resample
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        legal_pos = list(itertools.permutations(self.legalPositions, self.numGhosts))
        posNums = len(legal_pos)
        for pos in legal_pos:
            self.particles.extend([pos] * (num // posNums))
        for pos in legal_pos:
    ...
    def observeUpdate(self, observation: Union[int, float, None], gameState: busters.GameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                weight *= self.getObservationProb(observation[i], pos, particle[i], jailPos[i])
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(len(newParticle)):
                distribute = self.getPositionDistribution(gameState, newParticle, i, self.ghostAgents[i])
                newParticle[i] = distribute.sample()
            """*** END YOUR CODE HERE ***"""
