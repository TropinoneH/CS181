...
import util
from bayesNet import BayesNet, Factor
from factorOperations import eliminateWithCallTracking, joinFactors, joinFactorsByVariableWithCallTracking, normalize
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: BayesNet, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        factorsWithEvidence = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminatedVar in eliminationOrder:
            factorsWithEvidence, joinedFactor = joinFactorsByVariable(factorsWithEvidence, eliminatedVar)
            if len(joinedFactor.unconditionedVariables()) <= 1:
                continue
            eliminatedFactor = eliminate(joinedFactor, eliminatedVar)
            factorsWithEvidence.append(eliminatedFactor)
        fullJoint = joinFactors(factorsWithEvidence)
