...
def joinFactors(factors: List[Factor]):
    ...
    allVar = functools.reduce(lambda x, y: x | y, setsOfConditioned + setsOfUnconditioned)
    allUnconditionalVar = functools.reduce(lambda x, y: x | y, setsOfUnconditioned)
    resConditionalVar = allVar - allUnconditionalVar
    res = Factor(allUnconditionalVar, resConditionalVar, list(factors)[0].variableDomainsDict())
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        "*** YOUR CODE HERE ***"
        res = Factor(factor.unconditionedVariables() - {eliminationVariable}, factor.conditionedVariables(), factor.variableDomainsDict())
        for assignment in res.getAllPossibleAssignmentDicts():
            prob = 0
...
def normalize(factor: Factor):
    ...
    allAssignment = factor.getAllPossibleAssignmentDicts()
    ...
    for var in factor.unconditionedVariables():
        if len(variableDomainsDict[var]) > 1:
            ...
            conditionedVariable.append(var)
    res = Factor(unconditionedVariables, conditionedVariable, variableDomainsDict)
    for assignment in allAssignment:
